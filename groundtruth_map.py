...
def build(CONFIG, AOI, PLACES,
    ...
    *{box-sizing:border-box}html,body{margin:0;height:100%}
    ...
    .fig{position:absolute;inset:0;overflow:hidden}#map{position:absolute;inset:0;background:#0b0e13;z-index:1}
    ...
    .head{position:absolute;top:0;left:270px;right:170px;z-index:1100;padding:14px 18px 26px;pointer-events:none;
    ...
    .clock{position:absolute;top:12px;right:16px;z-index:1150;text-align:right;background:var(--panel);
    ...
    #leftRail{position:absolute;left:12px;top:12px;bottom:132px;z-index:1200;width:min(238px,42vw);
    ...
    .ctl{position:absolute;left:0;right:0;bottom:0;z-index:1150;display:flex;align-items:center;gap:12px;padding:13px 18px 15px;
    ...
    .leaflet-control-zoom{margin-bottom:140px!important;margin-right:16px!important;margin-top:12px!important}
    .leaflet-control-scale-line{background:rgba(0,0,0,.5)!important;color:#fff!important;border-color:#7d8798!important}
    #railBtn{position:absolute;left:16px;top:14px;z-index:1210;background:var(--panel);
    ...
    #fsBtn{position:absolute;right:16px;bottom:150px;z-index:1210;width:38px;height:38px;background:var(--panel);border:1px solid var(--line);color:#c3cad6;border-radius:9px;cursor:pointer;display:grid;place-items:center;padding:0}#fsBtn:hover{color:#fff;border-color:#48566c}#fsBtn svg{width:17px;height:17px}
    ...
    @media(max-width:760px){
      ...
      #leftRail{top:52px;width:min(240px,60vw)}
      .head{left:0;right:0;padding-top:52px;padding-left:64px}
    ...
    <div class="fig">
     ...
     <button id="fsBtn" title="Fullscreen" aria-label="Toggle fullscreen"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 9V4h5M20 9V4h-5M4 15v5h5M20 15v5h-5"/></svg></button>
    ...
    var map=L.map('map',{zoomControl:false,attributionControl:false,minZoom:8});
